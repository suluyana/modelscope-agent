    ...
    def _build_env(self, field: str, inherit: bool = False) -> Dict[str, str]:
        if inherit:
            ...
            env: Dict[str, str] = {
                ...
                # Never prompt on the TUI TTY (git/ssh password / host-key).
            ...
            'code_executor': [
                ...
                Tool(
                    ...
                    ('Execute shell commands locally under the workspace output directory (cwd). '
                     'Commands never inherit the agent TTY: no password prompts, no login shells. '
                     "For SSH use a remote command (ssh user@host 'uname -a') and key/ssh-agent auth. "
                     'Subject to policy (read_only vs workspace_write, network toggle). '
                     'Large stdout/stderr may be spilled to .ms_agent_artifacts. '
                     'Long commands that hit timeout are moved to the background with a task_id; '
                     'or pass run_in_background=true to return immediately.'
                     ),
                    parameters={
                        'type': 'object',
                        'properties': {
