...
class RepairAgent:
    ...
    def _repair_system_prompt() -> str:
        return (
            ...
            "\n"
            "## Import / dependency_missing fixes\n"
            "- Read the stack trace and fix files in import order (earliest failing frame first).\n"
            "- Use `grep` across `ms_agent/` for the same bare `import` / `from ... import` pattern.\n"
            "- Apply a consistent fix at EVERY matching site, not only the first file you find.\n"
            "- Do not stop after one edit — re-grep to confirm no bare imports of that module remain.\n"
            "- Prefer the same try/except + `install_package` pattern already used elsewhere in the repo.\n"
            "\n"
            "## When done\n"
